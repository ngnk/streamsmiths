...
def prepare_timeseries(df: pl.DataFrame, target_metric: str = "view_count") -> pd.DataFrame:
    ...
    if target_metric == 'engagement_rate':
        ...
        agg_df = df_pd.groupby('time_bin').agg({target_metric: 'sum'}).reset_index()
...
def main():
    ...
    # 3. Train/test split
    test_size = min(14, len(ts_data) // 5)
    ...
    ext_forecast_timestamps = pd.date_range(start=last_dt, periods=hours_to_forecast + 1, freq='h')[1:]
    ...
    try:
        ...
        dlm_full.fit(y_full)
        f_ext, f_vars = dlm_full.forecast(steps=hours_to_forecast)
    ...
    try:
        ...
        nn.fit(y_full)
        f_ext, f_std = nn.forecast(y_full, hours_to_forecast)
