...
class YouTubeBronzeIngestionV2:
    """
    Enhanced YouTube ingestion for Social Blade-style analytics:
    ...
    def ingest_trending_videos(self) -> Optional[Path]:
        ...
        if not video_ids:
            print("[BRONZE-V2] No trending videos found.")
            return None
...
def get_channel_ids_v2() -> list[str]:
    """
...
def run_bronze_ingestion_v2(channel_ids: list[str] | None = None, include_trending: bool = True) -> None:
    """
    Enhanced bronze ingestion V2 with better data capture and trending support.
    Dumps to separate directory (bronze_data_v2) to preserve existing progress.
    """
    load_dotenv()
    ...
    BRONZE_V2_DIR.mkdir(parents=True, exist_ok=True)

    bronze = YouTubeBronzeIngestionV2(
        api_key=api_key,
        output_dir=str(BRONZE_V2_DIR),
    ...
    if channel_ids is None:
        channel_ids = get_channel_ids_v2()

    print(f"[BRONZE-V2] Starting ingestion for {len(channel_ids)} channels...")
    all_results: list[Path] = []

    # Ingest channel data
    for idx, channel_id in enumerate(channel_ids, 1):
        print(f"[BRONZE-V2] ({idx}/{len(channel_ids)}) Ingesting channel {channel_id}...")
        try:
            path = bronze.ingest_channel_data(channel_id)
            all_results.append(path)
            print(f"[BRONZE-V2] ✓ Saved channel data to {path.name}")
        except Exception as e:
            print(f"[BRONZE-V2] ✗ Error ingesting {channel_id}: {e}")

    # Ingest trending videos
    if include_trending:
        print(f"[BRONZE-V2] Ingesting trending videos (region: US)...")
        try:
            trending_path = bronze.ingest_trending_videos()
            if trending_path:
                all_results.append(trending_path)
                print(f"[BRONZE-V2] ✓ Saved trending data to {trending_path.name}")
        except Exception as e:
            print(f"[BRONZE-V2] ✗ Error ingesting trending videos: {e}")

    print(f"\n[BRONZE-V2] ✅ Ingestion complete! Processed {len(all_results)} files.")
